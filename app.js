...
// Judge sets a question — resets round completely
async function setQuestion(code, q) {
  await dbSet(`${roomPath(code)}/question`, q);
...
// Judge starts the round — BOTH teams can buzz, no team pre-selection
async function startRound(code) {
  const timerEnd = Date.now() + 15000; // 15 seconds
  await dbSet(`${roomPath(code)}/round`, {
    phase: 'active',
    timerEnd,
    buzzedBy: null,
...
async function buzzIn(code, playerName, team) {
  ...
  // Allow buzz in 'active' phase (open to all) or 'passed' phase (only other team)
  if (round.phase !== 'active' && round.phase !== 'passed') return false;
  if (round.buzzedBy) return false; // someone already buzzed
  if (round.phase === 'passed' && round.passedFrom === team) return false; // can't buzz if your team already failed
...
// Clear round, keep question null
